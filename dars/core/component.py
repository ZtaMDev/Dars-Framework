...
class ComponentQuery:
    ...
    def find(
        ...
        predicate: Optional[Callable[["Component"], bool]] = None,
    ) -> "ComponentQuery":
    ...
    def attr(self, **attrs) -> "ComponentQuery":
        ...
        for component in self.components:
            for key, value in attrs.items():
                ...
                if key == "style":
                    component.style.update(Component._normalize_style_input(value))
                    continue
                elif key == "class_name":
...
class Component(ABC):
    def __init__(self, **props):
        ...
        # Handle style parsing (str / hook-like / mapping -> dict)
        self.style: Dict[str, Any] = self._normalize_style_input(props.get("style"))
        self.hover_style: Dict[str, Any] = self._normalize_style_input(
            props.get("hover_style")
        )
        self.active_style: Dict[str, Any] = self._normalize_style_input(
            props.get("active_style")
        )
    ...
    @staticmethod
    def _normalize_style_input(value: Any) -> Dict[str, Any]:
        """Normalize style input to a CSS dict.

        Supports dictionaries, utility-class strings, and hook-like objects
        that can be converted to string markers.
        """
        if value is None:
            return {}
        ...
        if isinstance(value, str):
            return parse_utility_string(value)
        # Hook-like values (useValue/useDynamic/useVRef/setVRef) are stringifiable
        # markers in Dars; parse as utility string when possible.
        return parse_utility_string(str(value))
        ...
        try:
            ...
            if not isinstance(handler, Script):
                if callable(handler):
                    ...
                    handler = dScript(cast(Any, handler.__code__))
        except Exception:
            # Best-effort: mantener como está (string o callable)
            pass
        return handler

    def set_event(self, event_name: str, handler: Any):
    ...
    def add_child(self, child: "Component"):
        if isinstance(child, type) and issubclass(child, Component):
            raise TypeError(
                f"The class {child.__name__} was passed instead of an instance. You should use {child.__name__}(...)."
            )
        child.parent = self
    ...
    def attr(self, **attrs) -> Union["Component", "DeferredAttr", dict]:
        ...
        if attrs:
            ...
            for key, value in attrs.items():
                if key == "style":
                    self.style.update(self._normalize_style_input(value))
                    continue
                elif key == "hover_style":
                    self.hover_style.update(self._normalize_style_input(value))
                    continue
                elif key == "active_style":
                    self.active_style.update(self._normalize_style_input(value))
                    continue
                elif key == "class_name":
...
class DeferredAttr:
    ...
    def render_children(self, exporter: "Exporter") -> str:
        """Render all children of the wrapped component using the exporter."""
        children_html = ""
    ...
    class DynamicFunctionComponent(Component):
        ...
        def __init__(self, *args, **kwargs):
            ...
            self.template_kwargs = (
                kwargs  # Store original kwargs to check for overrides
            )
            ...
            if _children:
                ...
                items = (
                    _children if isinstance(_children, (list, tuple)) else [_children]
                )
