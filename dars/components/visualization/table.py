...
class DataTable(Component):
    ...
    def _normalize_data(self, data, index, columns):
        ...
        # Check if it behaves like a DataFrame (avoid hard dependency import)
        try:
            if hasattr(data, "to_dict") and hasattr(data, "columns"):
                if index and hasattr(data, "reset_index"):
                    ...
                    normalized.append(
                        {
                            "key": col.get("key", ""),
                            "label": col.get(
                                "label", col.get("key", "").replace("_", " ").title()
                            ),
    ...
    def _render_table(
        ...
        border_style = (
            f"1px solid {theme_colors['border_color']}" if bordered else "none"
        )
        ...
        if header:
            ...
            for col in cols:
                header_cells.append(
                    f'<th style="padding: {padding}; text-align: {col["align"]}; background-color: {theme_colors["header_bg"]}; color: {theme_colors["header_color"]}; border-bottom: 2px solid {theme_colors["header_border"]}; border-right: {border_style}; font-weight: 600; white-space: nowrap;">{col["label"]}</th>'
                )
        ...
        for i, row in enumerate(rows):
            bg_color = (
                ...
                if i % 2 == 0
                else theme_colors["row_odd_bg"]
            )
            if not striped:
                bg_color = theme_colors["row_even_bg"]

            hover_style = (
                f"onmouseover=\"this.style.backgroundColor='{theme_colors['row_hover_bg']}'\" onmouseout=\"this.style.backgroundColor='{bg_color}'\""
                if hover
                else ""
            )
            ...
            for col in cols:
                ...
                cells.append(
                    f'<td style="padding: {padding}; text-align: {col["align"]}; border-bottom: {border_style}; border-right: {border_style}; color: {theme_colors["text_color"]};">{formatted_value}</td>'
                )

            body_rows.append(
                f'<tr style="background-color: {bg_color}; transition: background-color 0.2s;" {hover_style}>{"".join(cells)}</tr>'
            )
        ...
        if self.style:
            if isinstance(self.style, dict):
                container_style += "; " + "; ".join(
                    [f"{k}: {v}" for k, v in self.style.items()]
                )
            else:
