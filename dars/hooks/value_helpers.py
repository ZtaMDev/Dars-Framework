...
class MathExpression:
    ...
    def __init__(self, left, operator: Any, right):
        """
    ...
    def _to_structure(self) -> dict:
        ...
        """
        left = (
            self.left._to_structure()
            if hasattr(self.left, "_to_structure")
            else self.left
        )
        right = (
            self.right._to_structure()
            if hasattr(self.right, "_to_structure")
            else self.right
        )
        ...
        return {
            "op": "math_expr",
            "args": {"left": left, "operator": op_struct, "right": right},
        }
...
class BooleanExpression:
    ...
    def _to_structure(self) -> dict:
        ...
        return {
            "op": "bool_expr",
            "args": {"left": left, "operator": self.operator, "right": right},
        }
...
class ConditionalExpression:
    ...
    def _get_value_code(self, value) -> str:
        ...
        if isinstance(
            value, (ValueRef, MathExpression, BooleanExpression, ConditionalExpression)
        ):
    ...
    def _to_structure(self) -> dict:
        ...
        """
        cond = (
            self.condition._to_structure()
            if hasattr(self.condition, "_to_structure")
            else self.condition
        )
        true_v = (
            self.true_value._to_structure()
            if hasattr(self.true_value, "_to_structure")
            else self.true_value
        )
        false_v = (
            self.false_value._to_structure()
            if hasattr(self.false_value, "_to_structure")
            else self.false_value
        )

        return {
            "op": "cond_expr",
            "args": {"condition": cond, "true_val": true_v, "false_val": false_v},
        }
...
class ValueRef:
    ...
    def _to_structure(self) -> dict:
        ...
        if self._custom_code:
            return (
                self._custom_code
            )  # Should be a structure if possible, but keep fallback

        if self._is_state_path():
            parts = self.selector.split(".")
            base_val = {
                "op": "get_state_value",
                "args": {"state_id": parts[0], "prop_name": parts[1]},
            }
        else:
        ...
        if self._transform:
            return {
                "op": "transform",
                "args": {"input": base_val, "method": self._transform},
            }
...
(async () => {{
    ...
    def __eq__(self, other) -> "BooleanExpression":  # pyright: ignore[reportIncompatibleMethodOverride]
        """
    ...
    def __ne__(self, other) -> "BooleanExpression":  # pyright: ignore[reportIncompatibleMethodOverride]
        """
...
def url(template: str, **kwargs) -> Any:
