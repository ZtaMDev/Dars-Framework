...
class ValueMarker:
    ...
    def __init__(self, state_path: str, selector: Optional[str] = None):
        """
    ...
    def __repr__(self):
        return (
            f"ValueMarker(state_path='{self.state_path}', selector='{self.selector}')"
        )
    ...
    def get_initial_value(self):
        ...
        try:
            ...
            state = next(
                (s for s in reversed(STATE_V2_REGISTRY) if s.component.id == state_id),
                None,
            )
            if not state:
                return ""
...
(async () => {{
    try {{
        ...
        return dScript(data={"op": "get_dom_value", "args": {"selector": selector}})


def useValue(state_path: str, selector: Optional[str] = None) -> ValueMarker:
    """
...
setattr(useValue, "select", cast(object, UseValueSelector.select))
